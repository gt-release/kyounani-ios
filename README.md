...
## 概要（何ができるか）
- 子ども向け Today 画面（**きょう / つぎ / あした**。あしたは翌日分のみ表示）
...
- 上部フィルタは青/ピンク/2人アイコン付きボタンで「息子 / 娘 / 両方」を切替。
  - フィルタはボタン領域全体でタップ判定します（文字/アイコン以外の余白でも反応）。
- 「つぎ」カードをタップすると読み上げ + 残り時間リング表示。
- 残り時間リングは1時間ごとに色を変えて表示し、1時間超は「あと◯じかん◯ふん」表記。
...
- 右上ガイドは `allowsHitTesting(false)` で重ね表示しており、2本指長押しのタッチをブロックしません。
- 親モードでイベントの追加・編集・削除、テーマ切替（Kid / High Contrast）が可能。
- 親モード解除後は右上に「＋」（予定追加）と「スライダー」（親ツール再入場）の2ボタンを表示。スタンプ追加やバックアップ等へ戻りやすくしています。
- 予定の追加・編集・削除は Calendarタブの日別詳細（DayDetail）で行います（親モード時のみ＋/編集/削除を表示）。
- Today右上の「＋」は Calendarタブへ自動移動して当日の日別詳細を開き、予定追加画面を表示します。
- 「ロック」で子どもモードに即復帰。
...
- Rescue 経由で親画面を開いた後は、同一アンロックセッション中でも右上「＋」で予定追加導線を維持します。
...
- Today起点のクイック追加は常に当日を選択して追加画面を開くように修正（前回選択日の誤流用を防止）。
- 終日ON/OFF切替時も開始/終了の整合チェックを行い、OFFへ戻した際に意図しない5分予定になる問題を防止。
...
- 祝日CSVは `2024/5/5` のようなゼロ埋めなし日付も読み込み時に正規化して扱います。
