...
- Todayホームを「きょう / つぎ / あした」の3セクションで最適化（大見出し・広い余白・スタンプ優先表示）
- Design Tokensベースの `KyounaniTheme` を導入（Kid / High Contrast プリセット、色/余白/角丸/影/最小タップ領域を統一）
- Today/Calendar の常設2タブ導線（迷わず往復できる）
- Todayの空状態表示（今日0件 / 次予定なし）
...
- 祝日/土日色分け（weekday/weekend/holiday/selected/today をテーマトークンで統一）
...
## テーマ切り替え（親モード）
- 親モードの「見やすさ」セクションで **Kid / High Contrast** を切り替えできます。
- 設定は `UserDefaults` に保存され、次回起動後も維持されます。
- 子どもモード（Today / Calendar / DayDetail）にも同じテーマが適用されます。

## 子ども向け操作（Today / Calendar）
- 起動直後は **Todayタブ** を表示します。
- 上部の子どもフィルタ（息子 / 娘 / 両方）は常時表示され、どの表示対象かをすぐ切り替えできます。
- Todayは3セクションです。
  - 文字サイズは Dynamic Type に追従しつつ、子ども向けに極端に小さくならない下限を持たせています。
  - **きょう**: スタンプを大きく表示（最大2件 +N）。
  - **つぎ**: カード全体をタップすると、日本語読み上げ + TimerRing を同時に開始。
  - **あした**: 次の日の予定をスタンプ中心で表示。
- 空状態も子ども向けに表示します。
  - 今日の予定が0件: スタンプ + 短い文言を表示。
  - 次の予定がない: 「きょうは おしまい」を表示。
- 画面下の **Today / Calendar タブ** でいつでも往復できます。
- 子どもモードでは親モードボタンを表示せず、右上の隠しジェスチャ領域のみ有効です。
...
起動直後に子ども向け Today ホームが表示され、画面下タブで Calendar へ迷わず移動できます。
