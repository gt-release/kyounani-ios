...
- ペアレンタルゲート（2本指2秒長押し起点）: **対応強化（信頼性改善）**
  - 右上の固定領域に見た目より大きい透明ホットエリア（約120x120 + padding）を重ね、「親モード / 右上を2本指で2秒」ガイドを常時表示（ガイド側は `allowsHitTesting(false)` で判定を阻害しない）。
  ...
  - 親ゲート起動要求は短時間連打をデバウンスし、フォールバック多重発火時の不安定化を抑制。
