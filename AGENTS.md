...
- 親モードCRUD/繰り返し例外UI/スタンプ管理/テンプレ: **部分対応**
  - 簡易一覧・削除・クイック追加のみ。
  - 本格CRUD、スタンプ管理は未実装。
  - 日別詳細で繰り返し予定の例外編集3択UI（この日だけ/以降/全体）は実装済み（親モード限定・最小）。
...
- 例外編集UI（この日だけ/以降/全体）: **対応済み（最小 / 親モード限定）**
  - 例外由来シリーズを全体編集した際の重複生成を防ぐ保存挙動に修正済み。
...
## 14. 次の優先実装（推奨）
1. ペアレンタルゲート起点の実装（3本指2秒長押し）＋子どもモードから親導線を非表示。
2. 例外編集UIの拡張（この日だけ削除導線、入力項目追加）。
3. 親モードCRUDの拡張（本格CRUD、スタンプ管理のUX改善）。
4. 暗号化エクスポート（任意）。
