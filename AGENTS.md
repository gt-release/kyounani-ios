...
- カレンダートークンの差し替えポイント: **対応済み（準備）**
  - `DayCellEventTokensView.renderEventToken(event:)` に描画責務を集約。
...
2. スタンプ管理（初期セット、取り込み、正方形トリミング、保存）。
3. SwiftData Repository実装と移行。
4. カレンダーUI改善（スタンプ画像化、アクセシビリティ、見た目最適化）。
